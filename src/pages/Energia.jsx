...
export const Energia = () => {
  ...
  async function handleRegister(e) {
    ...
    if (lot.empreendimento == "" || lot.quadra == "" || lot.numero == "" || ImageURL == "") {
    ...
    <>
      <div className='flex flex-col items-center'>
        ...
        <form className='w-[400px]' onSubmit={handleRegister}>
          <h1 className='text-white text-[1.4rem] text-center bg-black rounded p-2 mt-8'> Solicitanção de Energia</h1>
          ...
          <div className=" h-[30px] cursor-pointer rounded  mb-4">
            <input type='file' multiple='multiple' accept='image/*' className="text-white" id="dropzone-file" onChange={(e)=> setImagens(e.target.files[0])  }  />

          </div>
          <button className='bg-[#27272A] text-white border border-black w-full py-2 text-[1.1rem] font-bold' >Enviar</button>

         {images !== "" && <img src={URL.createObjectURL(images)} className="w-auto " alt="" /> }


        </form>
      </div>
      {lot.empreendimento !== "" ? (
        <Pedido imagem={lot.imagem} empreendimento={lot.empreendimento} quadra={lot.quadra} numero={lot.numero} status={lot.status} pedido={lot.pedido} />
      ) :
        (<div></div>)
      }
    </>
